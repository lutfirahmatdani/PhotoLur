...
<body>
  ...
  <div class="gallery-container">
    <?php foreach ($foto as $row) : ?>
      <button class="btn btn-danger" onclick="hapusdarialbum('<?= $album[0]['id_album'] ?>/<?= $row[0]['id_foto'] ?>')">X</button>
      <div class="gallery-item mt-3" onclick="visitfoto('<?= $row[0]['id_foto'] ?>')">
        <img src="/foto_storage/<?= $row[0]['lokasi_file']; ?>">
      </div>
    <?php endforeach; ?>
  </div>
  </div>
